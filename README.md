...
`test.py` - Parses generic_metasyntax.gms using the definitions in gms.py, generates a grammar for Generic Metasyntax from the resulting parse tree, and uses the generated grammar to parse generic_metasyntax.gms again. Equivalency of production rules is checked, proving the 'parse, generate grammar, parse, generate grammar...' cycle can be repeated infintely. By extension, this proves that the metasyntax represents itself
